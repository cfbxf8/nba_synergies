...
def get_top_time_matchups(df):
    i_ = df[['i_lineup', 'i_time']]
    j_ = df[['j_lineup', 'j_time']]
    i_.columns = ['lineup', 'time']
    j_.columns = ['lineup', 'time']
    comb_ = pd.concat([i_, j_])
    comb_sum = comb_.groupby('lineup').sum()
    comb_sum.sort_values('time', ascending=False, inplace=True)
    comb_sum_25 = comb_sum[:int(len(comb_sum) * 0.25)+1]
    comb_sum_25.reset_index(inplace=True)
    V = set()
    for row in X.iterrows():
        V |= set(row[1]['lineup'])

    V = set()
    for row in new_X.iterrows():
...
if __name__ == '__main__':
    ...
    X = add_date(X)
    # X.sort_values('i_time', ascending=False, inplace=True)
    # X = X[:int(len(X) * 0.25) + 1]
